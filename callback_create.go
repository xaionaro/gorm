			...
			if scope.changeableField(field) {
				if field.IsNormal {
					...
					for _, dbName := range relationship.ForeignDBNames {
						if relationField := fields[dbName]; (relationField != nil) && !scope.changeableField(relationField) { // TODO: find out why "relation" can be null (it's connected with structs with interface{}-es)
